...
#   scripts/test/render/render.sh [--write-golden] [--png DIR] [--dump] [scene…]
#
...
exec docker run --rm --platform linux/arm64 -v "$ROOT":/src:rw $pngmount \
    ...
    -e U60_DEVUI_CJK_FONT=/fonts/dev/ZTEZhengYuan.ttf -e U60_DEVUI_ROBOTO=/fonts/dev/Roboto.ttf \
...
for s in "$@"; do for t in light dark; do
    ...
    p=; [ -n "$png" ] && p=--png=/png
    timeout 120 ./'"$BIN"' --scene=$s --theme=$t $mode=$g --expect=tests/render/expect.txt $p $dump 2>/tmp/err.$s.$t
    r=$?
