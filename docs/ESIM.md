...
真切换会断网，对只能经设备自己蜂窝出口连过去的机器尤其危险：切到没流量的
profile 就再也连不上了。所以流程验证用假 agent `scripts/esim-mock-agent.lua`（lua + nixio，设备自带）：
...
在测试设备上照这个走过：亮起 → 确认 → 切换中计秒 → 完成、失败、网页端并发、agent 中途重启、
agent 挂掉与恢复、浅色主题。真机上用真卡在触屏上从 CTM 切到 CMLINK：5 秒完成、没重启，
