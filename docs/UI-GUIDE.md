...
- HTML / CSS 仍然按文件热加载，`adb push` 完不用重启，约 1 秒内自动生效；状态数据则来自本机 HTTP/SSE 缓存。
...
- `menu.html` 和 `lockscreen.html` 都是特殊页：`menu.html` 电源键长按覆盖显示，放关机/重启/取消；`lockscreen.html` 在超时/电源键锁屏时全屏显示 PIN 键盘（解锁正确才回到界面），系统页锁屏开关从关→开时也用它设置新密码。两者都按全屏覆盖层处理，不继承普通页面的竖向滚动位置。
