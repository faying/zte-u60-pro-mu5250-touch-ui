...
extern void        touch_input_set_rotate180(int on);
static int         g_rot180 = 1;      /* 面板方向，启动时按配置定 */
...
static int path_is_esim(const char *path)
{
    const char *base = strrchr(path, '/');
    if (!base) base = path; else base++;
...
/* 磁贴副标题来自页面的 <meta name="description" content="...">，没写就用默认文案 */
static int function_desc_from_html(const char *html, char *dst, size_t cap)
{
    const char *p, *c, *end;
    size_t l;
    if (!html || !dst || cap == 0) return 0;
    p = strcasestr(html, "name=\"description\"");
    if (!p) return 0;
    end = strchr(p, '>');
    c = strcasestr(p, "content=\"");
    if (!c || (end && c > end)) return 0;
    c += 9;
    end = strchr(c, '"');
    if (!end) return 0;
    l = (size_t)(end - c);
...
static void function_title_for_file(char *dst, size_t cap, char *desc, size_t dcap,
                                    const char *path, const char *name)
{
    char *html = read_file(path);
    int have_title = 0;
    snprintf(desc, dcap, "%s", "自定义页面");
    if (html) {
        have_title = function_title_from_html(html, dst, cap);
        (void)function_desc_from_html(html, desc, dcap);
        free(html);
    }
    if (!have_title) function_label_from_name(dst, cap, name);
}
...
static const char *custom_function_tiles_html(void)
{
    ...
    for (int i = 0; i < n; i++) {
        char path[300], raw[96], rawdesc[96], title[160], desc[160], href[160];
        snprintf(path, sizeof path, "%s/%s", FUNCTIONS_DIR, names[i]);
        function_title_for_file(raw, sizeof raw, rawdesc, sizeof rawdesc, path, names[i]);
        html_esc(title, sizeof title, raw);
        html_esc(desc, sizeof desc, rawdesc);
...
static int build_kv(struct kv *t, const char *path)
{
    ...
    /* 首页 Tailscale 卡片：只读缓存，LocalAPI 在主循环的 tailscale_poll 里读 */
    t[i++] = (struct kv){ "TAILSCALECARD", tailscale_card_html(g_lock_state == 1) };
    ...
    t[i++] = (struct kv){ "ES_STATE", esim_state() };
    t[i++] = (struct kv){ "ES_LIST",  esim_list_html() };
    return i;
}
...
static void maybe_dump_fb(drm_disp_t *d)
{
    ...
    for (int y = 0; y < H; y++) {
        for (int x = 0; x < W; x++) {
            uint16_t px = g_rot180 ? d->fb[(size_t)(H - 1 - y) * pp + (W - 1 - x)]
                                   : d->fb[(size_t)y * pp + x];
...
/*
 * 正装面板的版本：目标行不倒序、行内也不倒序。
 * 起点与步进由 g_rot180 决定，其余逻辑两种方向共用。
 */
static inline uint16_t *copy_fwd_span(uint16_t *dp, const uint16_t *src, int n)
{
    for (int i = 0; i < n; i++) *dp++ = src[i];
    return dp;
}

static inline void fill_fwd_span(uint16_t *dp, uint16_t px, int n)
{
    while (n-- > 0) *dp++ = px;
}

/* 某一行在帧缓冲里的写入起点 */
static inline uint16_t *row_start(uint16_t *fb, int y, int rows, int pitch_px, int width)
{
    return g_rot180 ? fb + (size_t)(rows - 1 - y) * pitch_px + (width - 1)
                    : fb + (size_t)y * pitch_px;
}
...
static void compose_frame(drm_disp_t *d, int o)
{
    ...
    for (int y = SBAR_H; y < H; y++) {
        uint16_t *dp = row_start(d->fb, y, H, pp, W);
        ...
        if (g_rot180) {
            ...
            if (left > 0) dp = copy_fwd_span(dp, lr + o, left);
            if (o > 0)    copy_fwd_span(dp, rr, o);
        }
    }
...
static void scroll_blit(drm_disp_t *d, int scroll)
{
    ...
    for (int y = SBAR_H; y < Hh; y++) {
        ...
        uint16_t *dp = row_start(d->fb, y, Hh, pp, W);
        if (g_rot180) { if (src) copy_rev_span(dp, src, W); else fill_rev_span(dp, 0, W); }
        else          { if (src) copy_fwd_span(dp, src, W); else fill_fwd_span(dp, 0, W); }
    }
    html_view_set_scroll(scroll);
    draw_charts();
...
int main(void)
{
    ...
    g_rot180 = devui_rotate180();     /* U60 面板倒装，TopFlow 正装 */
    touch_input_set_rotate180(g_rot180);
    html_view_init(disp.fb, disp.width, disp.height, disp.pitch_px, g_rot180, resolve_ui_font());
    ...
    int swallow_tap = 0;   /* 1 = 本次按压是为了停住惯性滚动，释放时不当点击 */
    ...
    while (g_run) {
        ...
        if (!on_now) {
            ...
            int inertia_at_start = scroll_inertia;
            if (pressed && scroll_inertia) {
                /* 手指按下去是为了停住惯性滚动，不是点列表项：这一整次按压都不产生点击 */
                scroll_inertia = 0;
            ...
            if (!pressed && !prev_press && !dragging && !menu) {
                ...
                if (!queued_motion && inertia_at_start) {
                    /* 惯性滚动中落下的一次快速点按（整个按压在两次轮询之间完成）：
                     * 用户意图是停住列表，不是选中手指下面那一项 */
                    int tx, ty;
                    if (touch_input_take_latest_tap(&touch, &tx, &ty)) {
                        scroll_inertia = 0; scroll_track_valid = 0; scroll_v = 0.0f;
                        touch_input_clear_taps(&touch);
            ...
            if (pressed && !prev_press) {
                ...
                int swallow = swallow_tap;   /* 每次释放都清零，别残留到下一次按压 */
                int dx = rel_dx;
                swallow_tap = 0;
                if (sliding) { save_conf(); /* persist brightness after drag */ }
                    ...
                    need_render = 1;
                } else if (dragging && (!release_was_tap || swallow)) {
                    /* 位移超过点按阈值（横向拖、中途丢帧）或本次按压只是为了停住滚动：不触发链接 */
                    ...
                    if (!strncmp(act, "act:", 4)) {
                        ...
                        if (!strcmp(a, "poweroff")) {
                            ...
                            static long arm;
                            if (arm && now - arm < 5000) {
                                if (devui_restore_stock()) {
                                    /* 后台已排好启动；立刻退出把 DRM 让出去 */
                                    screen_off(&disp);
                                    exit(0);
                                }
                                arm = 0;
                                snprintf(g_toast, sizeof g_toast, "切换失败，已取消");
                        ...
                        else if (!strncmp(a, "esim:", 5)) {
                            int r = esim_select(atoi(a + 5));   /* 两段式：第一次只亮起 */
                            /*
                             * 亮起、点到正在用的那张都不弹提示：行本身已经变样。提示框正好盖在
                             * 列表中间，点击命中的是提示框而不是下面那一行，确认的第二下会被吃掉
                             * ——所以亮起时连残留的提示框也清掉。
                             */
                            if (r == ESIM_SEL_ARMED || r == ESIM_SEL_CURRENT) {
                                g_toast[0] = 0;
                            } else {
...
action_done:
        ...
        /* 首页 Tailscale 卡片（含锁屏预览）：首页亮着时才读 LocalAPI（tailscale_poll 自己节流） */
        if (!dragging && !scroll_inertia) {
            int ts_on = !menu && backlight_is_on() && path_is_signal_home(CUR_PATH);
            if (tailscale_poll(ts_on) && ts_on) need_render = 1;
        }

        /* eSIM 页：页面开着、或本机发起的切换还没出结果时才去问 zte-agent（esim_poll 自己节流） */
        if (!dragging && !scroll_inertia) {
